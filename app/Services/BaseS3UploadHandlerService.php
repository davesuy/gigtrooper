...
class BaseS3UploadHandlerService
{
	...
	public function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		$this->prefix= config('filesystems.prefix') . "/$fileuploadLabel/$fileuploadId/$handle/"; //note there is no
		// "folders"
		// in S3 though you can
		// prefix files with a
		// string that resembles a file system.
	...
	protected function get_file_objects($iteration_method = 'get_file_object') {
		...
		if (!is_dir($upload_dir)) {
			...
			try{
				...
				if(isset($result['Contents']) && count($result['Contents'])>0){
					foreach($result['Contents'] as $obj){
						...
						if($obj['Key']!=$this->prefix && strpos($obj['Key'], '/thumbnail/')===false){ //if Key is a full file path and not just a "directory"
							$a=pathinfo($obj['Key']);
			...
			if(!isset($file_names_arr)) $file_names_arr=array();
	...
	public function get_config_bytes($val) {
		...
		return $this->fix_integer_overflow($num);
	}
