...
{
    public function getCountriesDropDown($currentCountry, $key = 'filters', $all = true)
    {
        $initModel = new Country();
        ...
        $select = [];
        if ($all) {
            $select[] = new OptionData("All", 'ALL', false);
        }

        $options = $this->getOptionData($models, $currentCountry);
        $options = array_merge($select, $options);

        return view('fields.no-wrap-dropdown', [
            'name' => 'countryCode',
            'title' => "",
            'class' => "full-width",
            'options' => $options,
    ...
    protected function getOptionData($models, $value)
    {
        $options = [];
        ...
        if (!empty($models)) {
            foreach ($models as $model) {
                ...
                $val = new OptionData($model->title, $model->countryCode, $selected);
    ...
    public function getCountryNameByCode($code)
    {
        ...
        $name = (isset($countries[$code])) ? $countries[$code] : null;
    ...
    {
        $currency = \Countries::where('cca2', $code)->first()->currency->first();
